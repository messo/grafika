...
public:
	...
	Bounds getBounds(const Ray& ray) const {
		float a = ray.getV() * ray.getV();
		float b = 2 * (ray.getV() * (ray.getPoint() - origo));
		float c = origo * origo + ray.getPoint() * ray.getPoint()
				- 2 * (origo * ray.getPoint()) - radius * radius;
		float d = b * b - 4 * a * c;
		if (d < 0.0f) {
...
public:
	...
	Color getColor(const Vector& p) {
		...
		for (int j = 0; j < ATOMS_COUNT; j++) {
			if ((p - atoms[j].getOrigo()).length() / atoms[j].getRadius()
					< 1.0f) {
				...
				float w = (-12.0f * r2 * r2 + 34.0f * r2 - 22.0f) * 2.0f / 9.0f
						/ powf(atoms[j].getRadius(), 2.0f);
				c += atoms[j].getColor() * w;
				sum += w;
			}
		}
		return c / sum;
...
public:
	...
	float intersect(const Ray& ray) {
		...
		float t = (normal * (point - ray.getPoint())) / (normal * ray.getV());
...
class Scene {
	...
	Color trace(const Ray& ray, int iterat) {
		...
		if ((t = mol.intersect(ray)) >= 0.0f) {
			obj = &mol;
		} else if ((t = plane.intersect(ray)) >= 0.0f) {
			obj = &plane;
		}
		...
		if (obj != NULL) {
			...
			bool inShadow = false;
			if (mol.intersect(toLight, 0.0f, 9999999.99f, obj) >= 0.0f) {
				shade *= 0.15f;
				inShadow = true;
			}
...
public:
	...
	void init() {
		...
		Vector cameraPos(0.0f, 0.0f, near_ - maxY);
